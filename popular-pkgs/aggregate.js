...
sortedDL.splice(50000);
sorted.splice(50000);

const thead = "| Package | Dependents | Monthly Downloads | Weekly Downloads |\n| --- | --- | --- | --- |\n";
fs.writeFileSync(
  path.join(__dirname, "per-monthly_dl.md"),
  thead+sortedDL.map((p) => `| ${p.name} | ${p.dp} | ${p.dl.monthly} | ${p.dl.weekly} |`).join("\n")
);
fs.writeFileSync(
  path.join(__dirname, "per-dependents_count.md"),
  thead+sorted.map((p) => `| ${p.name} | ${p.dp} | ${p.dl.monthly} | ${p.dl.weekly} |`).join("\n")
);
